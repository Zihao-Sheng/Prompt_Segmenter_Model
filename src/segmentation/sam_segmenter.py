...
class SAMBoxSegmenter(BaseSegmenter):
    def __init__(self, config: dict, run_dir: Path, log=None):
        ...
        self.checkpoint_path = self._resolve_path(str(segmenter_cfg.get("sam_checkpoint_path", "")))
        self.model_type = str(segmenter_cfg.get("sam_model_type", "vit_b"))
        self.predictor = None
    ...
    def _initialize(self) -> None:
        if self.checkpoint_path is None or not self.checkpoint_path.exists():
            self.warning = "Warning: SAM checkpoint is missing. Continuing with boxes only."
            return
        try:
            import torch
            ...
            self.warning = f"Warning: SAM backend unavailable ({exc}). Continuing with boxes only."
    ...
    def segment(self, frame, detections: list[Detection], frame_idx: int, save_mask_pngs: bool) -> list[SegmentationMask]:
        ...
        for detection in detections:
            ...
            if self._should_skip_mask_for_detection(detection):
                continue
            try:
                masks, scores, _ = self.predictor.predict(box=np.array(detection.bbox, dtype=np.float32), multimask_output=False)
            except Exception as exc:
                self.log(f"Warning: SAM predict failed for {detection.label} on frame {frame_idx} ({exc}).")
                continue
